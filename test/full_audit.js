...
try {
  ...
  while ((m = idRegex.exec(uiCode)) !== null) {
    queriedIds.add(m[1]);
  }

  let missingStaticIds = 0;
  queriedIds.forEach(id => {
    // Check if ID is in index.html or generated dynamically in ui.js
    const inHtml = html.includes(`id="${id}"`) || html.includes(`id='${id}'`);
    const inUi = uiCode.includes(`id="${id}"`) || uiCode.includes(`id='${id}'`) || id.includes('_') || id.includes('${');
    if (!inHtml && !inUi) {
